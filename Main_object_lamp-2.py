...
@ti.kernel
def ComputeCollsion_2(T: ti.f64):
    for i in pointLocation:
        # 中间圆柱
        if pointLocation[i][2] < circularTH2-1 and pointLocation[i][2] > circularTH2-circularHeight2+1:
            tempcenter = ti.Vector([cylinderCenter2[0], cylinderCenter2[1], pointLocation[i][2]], dt=ti.f64)
            Dir3 = pointLocation[i] - tempcenter
            nD = Dir3/Dir3.norm()
            if Dir3.norm()>T:
                pointLocation[i] = tempcenter+(T-1e-4)*nD
...
def Init():
    pc_array=read_ply("InputModel.ply")
    InitTi_2(pc_array)
    Index = 0
...
def Step(judge:bool,RID:float):
    for i in range(150):
    # print(pointLocation)
        if(judge):
            ComputeCollsion_2(RID)
    # print(pointLocation)
        RecordLocation()
        ComputeForce()
        Forward(2e-6)
...
def main():
    ...
    try:
        while True:
            # gui.show()
            if Frame<5000:
                Step(True,1.2+0.002*(5000-Frame))
            else:
                Step(False,0)
            Frame += 1
            if not Frame % 100:
